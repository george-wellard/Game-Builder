...
{
	std::list<Node*> open; // Open Nodes
	std::list<Node> closed; // Closed nodes
	...
	std::cout << "Start Node : (" << start.getRow() << " " << start.getColumn() << ")" << std::endl;
	...
	while (!open.empty())
	{
		...
		if (current_node.equals(goal_node))
		{
			...
			std::cout << "Goal Node: Row:" << goal.getRow() << ", Column:" << goal.getColumn() << std::endl;
			return true;
		}
		...
		std::list<Node>::iterator closedIt; // Setting iterator so we can go through the closed list
		std::list<Node*>::iterator openIt; // Setting iterator so we can go through the open list
...
{
	std::list<Node>::iterator graphListIt; // Iterating through the graph
	...
	for (Node node : path)
	{
		std::cout << node.getRow() << " " << node.getColumn() << std::endl;
	}
}
